<?php
require "../PHP/cadastro/conexao.php";
session_start();

// 🔹 Supondo que você tenha o id do usuário na sessão
$id_usuario = $_SESSION['usuario_id'] ?? 1; // ajuste conforme sua sessão

// Buscar pontuação do usuário
$pontos = 0;
$sql = "SELECT pontos FROM desempenho WHERE id_usuario = ?";
$stmt = $conexao->prepare($sql);
$stmt->bind_param("i", $id_usuario);
$stmt->execute();
$stmt->bind_result($pontos_usuario);
if($stmt->fetch()){
    ...
    <div style="display:grid;grid-template-columns:2fr 1fr;gap:20px;margin-top:20px;">
      <div>
        ...
        <div class="card" style="margin-top:20px;">
         <button class="btn-cronograma"><a href="cronograma.html">Faça seu Cronograma</a></button>
        </div>
      </div>
      ...
      <table class="calendar">
        ...
        <tbody>
          <tr>
            <td></td><td>1</td><td>2</td><td class="today">3</td><td>4</td><td>5</td><td>6</td>
          </tr>
          <tr>
